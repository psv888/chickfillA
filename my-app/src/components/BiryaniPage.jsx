...
export default function BiryaniPage() {
  ...
  const addToCart = (dish) => {
    ...
    if (cartItems.length > 0 && cartItems[0].dish.parent_id !== dish.parent_id) {
      if (window.confirm('Your cart contains items from another biryani point. Would you like to clear it and add this item?')) {
        clearCart();
  ...
  const cartBiryaniPoint = biryani.find(b => b.id === cartItems[0]?.dish.parent_id);
  ...
  // Order Confirmed Animation with Confetti
  function OrderConfirmedAnimation({ onDone }) {
    // Show for 2.5 seconds, then call onDone
    React.useEffect(() => {
      const timer = setTimeout(() => {
        onDone && onDone();
      }, 2500);
    ...
    return (
      <div style={{
        ...
        height: '100vh',
        zIndex: 9999,
        background: 'linear-gradient(135deg, #fff7ed 0%, #ffe3e3 100%)',
        display: 'flex',
        alignItems: 'center',
  ...
  // Order Blast Animation (like login success, but with 🎉🎊 and order confirmed lottie)
  const blastIcons = ['🎉', '🎊', '😁', '🤩'];
  function OrderBlastAnimation({ onAnimationEnd }) {
    const [confettiBursts, setConfettiBursts] = React.useState([]);
    const burstDuration = 5 * 1000; // 8 seconds
  ...
  // Status map for progress bar and text
  const statusMap = {
  ...
  // Parse GeoJSON location to [lat, lon]
  ...
  // Real-time subscription for tracking modal
  useEffect(() => {
  ...
  return (
    <div className="register-bg food-bg-animate" style={{minHeight:'100vh', width:'100vw', position:'relative'}}>
      ...
      >
        ...
        {cartBiryaniPoint && <div className="cart-restaurant-name">{cartBiryaniPoint.name}'s Biryani</div>}
        {checkoutStep === 'cart' && (
          cartItems.length === 0 ? (
