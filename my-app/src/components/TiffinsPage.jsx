...
export default function TiffinsPage() {
  ...
  const addToCart = (dish) => {
    ...
    if (cartItems.length > 0 && cartItems[0].dish.parent_id !== dish.parent_id) {
      if (window.confirm('Your cart contains items from another tiffin center. Would you like to clear it and add this item?')) {
        clearCart();
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Get all tiffin centers
        const { data: allTiffins, error } = await supabase
          .from('admin_items')
          .select('*')
          .eq('section', 'tiffins');
        if (error) {
          setTiffins(allTiffins || []);
          return;
        }
        ...
        const filteredTiffins = (allTiffins || []).filter(r => {
          if (r.latitude == null || r.longitude == null) return false;
          const dist = getDistanceKm(userLatLng.lat, userLatLng.lon, r.latitude, r.longitude);
          r.distance = dist;
          return dist <= 10;
        });
        filteredTiffins.sort((a, b) => (a.distance || 0) - (b.distance || 0));
  ...
  const cartTiffinCenter = tiffins.find(t => t.id === cartItems[0]?.dish.parent_id);
        ...
        handler: async function (response) {
          const orderResponse = await fetch('https://chickfilla.onrender.com/api/create-order', {
            ...
            body: JSON.stringify({
              ...
              restaurant_id: cartTiffinCenter?.id,
            })
          });
  ...
  return (
    <div className="register-bg food-bg-animate" style={{minHeight:'100vh', width:'100vw', position:'relative'}}>
      ...
      >
        ...
        <div style={{ overflowY: 'auto', height: 'calc(100% - 70px)', padding: '0 20px' }}>
        {menuModalParent && (
          <>
      ...
      >
        ...
        {cartTiffinCenter && <div className="cart-restaurant-name">{cartTiffinCenter.name}'s Tiffins</div>}
        {checkoutStep === 'cart' && (
          cartItems.length === 0 ? (
                ...
                {cartItems.map((item) => (
                  <div key={item.dish.id} className="cart-item">
                    ...
                    <div className="cart-item-controls">
                      ...
                      <button onClick={() => originalAddToCart(item.dish)} className="quantity-btn">
                        <span className="material-icons">add</span>
                      </button>
                    </div>
                  </div>
        ...
        {orderTrackingLoading ? (
          ...
          <div className="order-status-container">
            ...
            {orderTrackingOrder.status === 'out_for_delivery' && deliveryLocation && (
              (() => { console.log('orderTrackingOrder:', orderTrackingOrder); console.log('deliveryLocation:', deliveryLocation); })(),
